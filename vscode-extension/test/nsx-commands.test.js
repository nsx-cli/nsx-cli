...
describe('NSX VS Code extension helpers', () => {
  it('builds CLI args without duplicating CLI logic', () => {
    ...
    expect(
      buildFixArgs({ dryRun: true, outputPath: 'c:/tmp/report.md' }),
    ).toEqual(['fix', '--dry-run', '--output', 'c:/tmp/report.md']);
    expect(buildGraphArgs()).toEqual(['graph']);
    expect(buildDocsGenerateArgs()).toEqual(['docs', 'generate']);
    expect(buildGenerateArgs('module', 'users')).toEqual([
      'generate',
      'module',
      'users',
    ]);
    expect(buildMakeArgs('resource', 'users')).toEqual([
      'make',
      'resource',
      'users',
    ]);
    expect(buildPrismaCrudArgs('User')).toEqual(['prisma', 'crud', 'User']);
    expect(buildMarketplaceListArgs()).toEqual([
      'marketplace',
      'templates',
      'list',
    ]);
  ...
  it('derives a suggested name from explorer paths', () => {
    expect(
      deriveNameFromResourcePath('c:\\workspace\\src\\modules\\users'),
    ).toBe('users');
    expect(
      deriveNameFromResourcePath(
        'c:\\workspace\\src\\modules\\users\\users.controller.ts',
      ),
    ).toBe('users.controller');
  });
});
