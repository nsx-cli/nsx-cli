...
export class Bootstrap {
  public create(): ApplicationContext {
    ...
    const createModuleExecutor = new CreateModuleExecutor(generatorRegistry.get("module"));
    const createControllerExecutor = new CreateControllerExecutor(generatorRegistry.get("controller"));
    const createServiceExecutor = new CreateServiceExecutor(generatorRegistry.get("service"));
    const createRepositoryExecutor = new CreateRepositoryExecutor(generatorRegistry.get("repository"));
    const createEntityExecutor = new CreateEntityExecutor(generatorRegistry.get("entity"));
    const createCreateDtoExecutor = new CreateCreateDtoExecutor(generatorRegistry.get("dto"));
    const createUpdateDtoExecutor = new CreateUpdateDtoExecutor(generatorRegistry.get("dto"));
    const addProviderExecutor = new AddProviderExecutor(moduleCrudSupport, new AddProviderOperation());
    const addControllerExecutor = new AddControllerExecutor(moduleCrudSupport, new AddControllerOperation());
    const organizeImportsExecutor = new OrganizeImportsExecutor(moduleCrudSupport, new OrganizeImportsOperation());
    ...
    const templateMarketplaceService = new TemplateMarketplaceService(process.cwd(), fileService);
    const doctorAnalyzer = new DoctorAnalyzer(prismaLoader);
    const doctorFormatter = new DoctorReportFormatter();
    const doctorService = new DoctorService(new ProjectScanner(process.cwd()), doctorAnalyzer, doctorFormatter);
    const analyzeAnalyzer = new AnalyzeAnalyzer();
    const analyzeFormatter = new AnalyzeFormatter();
    const analyzeService = new AnalyzeService(new ProjectScanner(process.cwd()), analyzeAnalyzer, analyzeFormatter, fileService);
    ...
    const graphService = new GraphService(new ProjectScanner(process.cwd()), astProjectContext, graphBuilder, graphFormatter, fileService);
    const documentationCollector = new DocumentationCollector(new ProjectScanner(process.cwd()), fileService, prismaLoader, prismaDmmf);
    const documentationFormatter = new DocumentationFormatter();
    const documentationService = new DocumentationService(documentationCollector, documentationFormatter, fileService);
    ...
    const pluginConfigResolver = new PluginConfigResolver(configService, fileService, process.cwd());
    const pluginLoader = new PluginLoader(process.cwd());
    const pluginManager = new PluginManager(pluginConfigResolver, pluginLoader, process.cwd());
    ...
    const documentationCommand = new DocumentationCommand(documentationService, documentationFormatter);
    const aiCommand = new AiCommand(aiService);
    const pluginCommand = new PluginCommand(pluginManager);
    const marketplaceCommand = new MarketplaceCommand(templateMarketplaceService);
    ...
    applicationContext.register(ExecutionStepExecutorRegistry, executionStepExecutorRegistry);
    ...
    applicationContext.register(TemplateMarketplaceService, templateMarketplaceService);
    ...
    applicationContext.register(CreateControllerExecutor, createControllerExecutor);
    applicationContext.register(CreateServiceExecutor, createServiceExecutor);
    applicationContext.register(CreateRepositoryExecutor, createRepositoryExecutor);
    applicationContext.register(CreateEntityExecutor, createEntityExecutor);
    applicationContext.register(CreateCreateDtoExecutor, createCreateDtoExecutor);
    applicationContext.register(CreateUpdateDtoExecutor, createUpdateDtoExecutor);
    ...
    applicationContext.register(OrganizeImportsExecutor, organizeImportsExecutor);
