...
vi.mock("fs-extra", () => ({
  ...
  glob: vi.fn((pattern: string, cb: (error: Error | null, matches: string[]) => void) => cb(null, [pattern])),
...
describe("FileSystem", () => {
  ...
  it("propaga erro no glob", async () => {
    vi.mocked(fsExtra.glob).mockImplementationOnce(((pattern: string, cb: (error: Error | null, matches: string[]) => void) =>
