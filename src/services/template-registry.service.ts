...
export class TemplateRegistry {
  private readonly templateRoot = path.resolve(
    process.cwd(),
    'src',
    'templates',
  );
  private readonly marketplaceManifestPath = path.resolve(
    process.cwd(),
    '.nsx',
  ...
  async resolveTemplatePath(templateName: string): Promise<string> {
    const marketplaceTemplatePath =
      await this.resolveMarketplaceTemplatePath(templateName);
    ...
    const candidates = [
      ...
      path.join(
        ...
        `${this.templateFileName(normalizedTemplateName)}.hbs`,
      ),
    ...
    throw new Error(
      `Template '${templateName}' não encontrado. Verifique a pasta de templates ou o nome do template.`,
    );
  }

  private async resolveMarketplaceTemplatePath(
    templateName: string,
    ...
    const manifest = (await readJson(
      this.marketplaceManifestPath,
    )) as MarketplaceManifest;
    const marketplacePath =
      manifest.overrides?.[this.normalizeTemplateName(templateName)];
