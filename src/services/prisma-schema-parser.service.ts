...
export class PrismaSchemaParser {
  ...
  parse(): { models: PrismaModel[]; enums: PrismaEnum[]; ast: PrismaNode[] } {
    ...
    const models = ast
      .filter((node): node is PrismaNode & { kind: "model" } => node.kind === "model")
      .map((node) => ({
        name: node.name ?? "",
        fields: node.fields ?? [],
    ...
    const enums = ast
      .filter((node): node is PrismaNode & { kind: "enum" } => node.kind === "enum")
      .map((node) => ({
        name: node.name ?? "",
        values: node.values ?? [],
        raw: node.raw ?? "",
...
class PrismaTokenParser {
  ...
  private parseModel(tokenStream: PrismaTokenStream): PrismaNode {
    ...
    while (!tokenStream.isEof()) {
      tokenStream.skipNewlines();
      if (tokenStream.peek().kind === "symbol" && tokenStream.peek().value === "}") {
        tokenStream.consume("symbol", "}");
        break;
      }

      if (tokenStream.peek().kind === "identifier" && tokenStream.peek().value.startsWith("@@")) {
  ...
  private parseFieldLine(tokens: PrismaToken[]): PrismaField | null {
    ...
    const typeToken = filtered[1]?.kind === "identifier" ? filtered[1].value : undefined;
    if (!typeToken) {
      return null;
    }
  ...
  private readParenthesizedValue(tokens: PrismaToken[], startIndex: number): string {
    let cursor = startIndex;
    while (cursor < tokens.length && !(tokens[cursor].kind === "symbol" && tokens[cursor].value === "(")) {
      cursor += 1;
    }
  ...
  private collectBracketList(value: string, key: string): string[] {
    ...
    while (cursor < value.length) {
      ...
      result.push(...inside.split(",").map((item) => item.trim()).filter(Boolean));
