...
describe('TemplateRegistry', () => {
  ...
  it('prioriza template instalado via marketplace', async () => {
    const tempRoot = await mkdtemp(
      path.join(os.tmpdir(), 'nsx-template-registry-'),
    );
    process.chdir(tempRoot);

    await mkdir(
      path.join(tempRoot, '.nsx', 'templates', 'enterprise-controller'),
      { recursive: true },
    );
    await mkdir(path.join(tempRoot, 'src', 'templates'), { recursive: true });

    const installedTemplatePath = path.join(
      tempRoot,
      '.nsx',
      'templates',
    ...
    await writeFile(
      path.join(tempRoot, 'src', 'templates', 'controller.hbs'),
      'default controller',
      'utf8',
    );
    await writeFile(
