...
describe("GeneratorRegistry", () => {
  ...
  it("registerMany/unregister e valida limpeza de alias/categoria", () => {
    ...
    registry.registerMany([g1 as never, g2 as never]);
  ...
  it("lança erros para duplicidade, não encontrado e metadata inválida", () => {
    ...
    registry.register(makeGenerator("resource", ["res"]) as never);

    expect(() => registry.register(makeGenerator("resource") as never)).toThrow(GeneratorAlreadyRegisteredException);
    expect(() => registry.register(makeGenerator("other", ["res"]) as never)).toThrow(GeneratorAlreadyRegisteredException);
