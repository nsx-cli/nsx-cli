...
export class DoctorAnalyzer {
  ...
  public async analyze(
    ...
    const controllers = this.collectClassesByDecorator(
      sourceFiles,
      'Controller',
    );
  ...
  private collectClassesByDecorator(
    sourceFiles: ReturnType<DoctorAnalyzer['getWorkspaceSourceFiles']>,
    ...
    return sourceFiles.flatMap((sourceFile) =>
      sourceFile
        ...
        .filter(
          (classDeclaration) =>
            classDeclaration.getDecorator(decoratorName) !== undefined,
        )
  ...
  private collectClassesBySuffix(
    sourceFiles: ReturnType<DoctorAnalyzer['getWorkspaceSourceFiles']>,
    suffix: string,
  ): NamedItem[] {
    return sourceFiles.flatMap((sourceFile) =>
      sourceFile
        ...
        .filter((classDeclaration) =>
          (classDeclaration.getName() ?? '').endsWith(suffix),
        )
        .map((classDeclaration) => ({
          filePath: sourceFile.getFilePath(),
          className:
  ...
  private findDuplicateProviders(
    sourceFiles: ReturnType<DoctorAnalyzer['getWorkspaceSourceFiles']>,
  ): ProviderDuplicateIssue[] {
  ...
  private async buildPrismaSection(
    projectInfo: ProjectScannerResult,
  ): Promise<DoctorSectionReport> {
        ...
        issues: [
          {
            ...
            message:
              error instanceof Error
                ? error.message
                : 'Erro desconhecido ao validar Prisma.',
          },
        ],
      };
    }
  ...
  private buildTypeScriptSection(
    projectInfo: ProjectScannerResult,
    ...
    return {
      ...
      summary:
        errors.length > 0
          ? `${errors.length} erro(s) de TypeScript encontrados.`
  ...
  private buildControllersSection(
    controllers: NamedItem[],
  ): DoctorSectionReport {
    return {
      name: 'Controllers',
      ...
      summary:
        controllers.length > 0
          ? `${controllers.length} controllers encontrados.`
  ...
  private buildServicesSection(services: NamedItem[]): DoctorSectionReport {
    return {
      ...
      summary:
        services.length > 0
          ? `${services.length} services encontrados.`
  ...
  private buildDuplicateProvidersSection(
    issues: ProviderDuplicateIssue[],
  ): DoctorSectionReport {
    return {
      ...
      summary:
        issues.length > 0
          ? `${issues.length} provider(es) duplicado(s) encontrado(s).`
  ...
  private buildUnusedImportsSection(
    issues: DoctorIssue[],
  ): DoctorSectionReport {
    return {
  ...
  private buildCircularDependenciesSection(
    issues: DoctorIssue[],
  ): DoctorSectionReport {
    return {
