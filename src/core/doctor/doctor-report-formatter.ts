...
export class DoctorReportFormatter {
  public format(report: DoctorReport, outputPath: string): string {
    ...
    lines.push(
      `- Duplicate providers: ${report.statistics.duplicateProviders}`,
    );
  ...
  public formatConsoleSummary(report: DoctorReport): string {
    ...
    lines.push(
      `Modules: ${report.statistics.modules} | Controllers: ${report.statistics.controllers} | Services: ${report.statistics.services}`,
    );
    lines.push(
      `Duplicated providers: ${report.statistics.duplicateProviders} | Unused imports: ${report.statistics.unusedImports} | Cycles: ${report.statistics.circularDependencies}`,
    );
