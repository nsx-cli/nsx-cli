...
describe('ApplicationContext', () => {
  it('registra, resolve e remove serviços', () => {
    ...
    expect(context.resolve<{ enabled: boolean }>('cache')).toEqual({
      enabled: true,
    });
  ...
  it('lança erro para token duplicado e token ausente', () => {
    ...
    expect(() => context.register('config', { a: 2 })).toThrow(
      'already registered',
    );
    ...
    expect(() => context.register(symbolToken, { a: 2 })).toThrow(
      'Service token already registered: demo',
    );
    ...
    expect(() => context.register(DemoService, new DemoService())).toThrow(
      'Service token already registered: DemoService',
    );
  });
