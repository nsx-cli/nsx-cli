...
describe("CrudGenerator", () => {
  it("carrega o Prisma, localiza o model e delega para o orchestrator", async () => {
    ...
    const executeMock = vi.fn().mockResolvedValue({ modelName: "User", totalSteps: 1 });
  ...
  it("lanca erro quando o model nao existe", async () => {
    ...
    const executeMock = vi.fn().mockResolvedValue({ modelName: "Missing", totalSteps: 0 });
    ...
    await expect(crudGenerator.generate("Missing")).rejects.toBeInstanceOf(CrudModelNotFoundException);
