...
describe('Create executors', () => {
  it.each([
    ...
    'delega para o generator correto em %s',
    async (ExecutorClass, stepType) => {
...
export class UserModule {}`,
  ...
  it('AddProviderExecutor delega para AddProviderOperation e salva o arquivo', async () => {
    ...
    const executor = new AddProviderExecutor(support as never, operation);

    expect(
      executor.supports({
        type: ExecutionStepType.ADD_PROVIDER,
        target: 'user',
      }),
    ).toBe(true);
  ...
  it('AddControllerExecutor delega para AddControllerOperation e salva o arquivo', async () => {
    ...
    const executor = new AddControllerExecutor(support as never, operation);

    expect(
      executor.supports({
        type: ExecutionStepType.ADD_CONTROLLER,
        target: 'user',
      }),
    ).toBe(true);
  ...
  it('OrganizeImportsExecutor organiza imports e salva o arquivo', async () => {
    ...
    const executor = new OrganizeImportsExecutor(support as never, operation);

    expect(
      executor.supports({
        type: ExecutionStepType.ORGANIZE_IMPORTS,
        target: 'user',
      }),
    ).toBe(true);
