...
describe("CrudExecutor", () => {
  it("executa os steps na ordem do plano", async () => {
    ...
    const executors = [
      {
        supports: (step: { type: ExecutionStepType }) => step.type === ExecutionStepType.CREATE_MODULE,
        execute: vi.fn(async (step: { type: ExecutionStepType; target: string }) => {
          calls.push(`${step.type}:${step.target}`);
        }),
      },
      {
        supports: (step: { type: ExecutionStepType }) => step.type === ExecutionStepType.ADD_PROVIDER,
        execute: vi.fn(async (step: { type: ExecutionStepType; target: string }) => {
          calls.push(`${step.type}:${step.target}`);
        }),
      },
  ...
  it("lanca erro quando nenhum executor suporta o step", async () => {
    ...
    const plan = new ExecutionPlan([{ type: ExecutionStepType.CREATE_MODULE, target: "user" }]);

    await expect(executor.execute(plan)).rejects.toThrow("No executor found for step: CREATE_MODULE");
  });
});
