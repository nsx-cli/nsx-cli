...
export class UserModule {}`
    ...
    expect(steps.some((step) => step.type === "RegisterMissingProviderOperation")).toBe(true);
    expect(steps.some((step) => step.type === "RegisterMissingControllerOperation")).toBe(true);
    expect(steps.some((step) => step.type === "FixModuleExportsOperation")).toBe(true);
    expect(steps.some((step) => step.type === "FixBarrelExportsOperation")).toBe(true);
    expect(steps.some((step) => step.type === "OrganizeImportsOperation")).toBe(true);
    expect(steps.some((step) => step.type === "RemoveUnusedImportsOperation")).toBe(true);

    const modulePath = "/workspace/src/modules/user/user.module.ts";
    const providerStep = steps.find((step) => step.type === "RegisterMissingProviderOperation");
