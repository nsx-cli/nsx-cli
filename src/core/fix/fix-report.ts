...
export class FixReportFormatter {
  public formatPreview(report: FixReport): string {
    ...
    const previewItems = report.executionPlan.map((step, index) => `${index + 1}. ${step.type} -> ${step.filePath}`);
  ...
  public formatMarkdown(report: FixReport, outputPath: string): string {
    ...
    lines.push("## Summary");
    lines.push("");
    lines.push(`- Planned operations: ${report.summary.totalPlannedOperations}`);
    ...
    if (report.executionPlan.length === 0) {
      ...
      for (const step of report.executionPlan) {
        lines.push(`- ${step.type}: ${step.description} (${step.filePath.replace(/\\/g, "/")})`);
      }
    }
    ...
    if (report.execution.length === 0) {
      ...
      for (const execution of report.execution) {
        ...
        lines.push(`- ${execution.status.toUpperCase()}: ${execution.step.type} (${execution.step.filePath.replace(/\\/g, "/")})${error}`);
      }
    }
    ...
    lines.push("");
    lines.push(`- Source files: ${report.analyzeReport.statistics.sourceFiles}`);
    lines.push(`- Average complexity: ${report.analyzeReport.statistics.averageComplexity}`);
    lines.push(`- Average coupling: ${report.analyzeReport.statistics.averageCoupling}`);
    lines.push(`- Average cohesion: ${report.analyzeReport.statistics.averageCohesion}`);
