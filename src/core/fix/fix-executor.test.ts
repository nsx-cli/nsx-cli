...
describe("FixExecutor", () => {
  it("executa todas operações com sucesso", () => {
    ...
    const projectContext = {
      getSourceFile: (filePath: string) => (filePath.endsWith("user.module.ts") ? moduleFile : undefined),
      getProject: () => project,
    };
    ...
    expect(barrel?.getExportDeclarations().map((entry) => entry.getModuleSpecifierValue())).toEqual([
      "./user.service",
      "./user.controller",
    ]);
  });
  ...
  it("captura falhas por arquivo ausente e dados inválidos", () => {
    ...
    expect(result.executedSteps.some((entry) => (entry.error ?? "").includes("Source file not found"))).toBe(true);
    expect(result.executedSteps.some((entry) => (entry.error ?? "").includes("Invalid step data"))).toBe(true);
    expect(result.executedSteps.some((entry) => (entry.error ?? "").includes("Unsupported fix operation"))).toBe(true);
  });
});
