...
describe("PluginManager", () => {
  ...
  it("inicializa plugins e segrega carregados/falhos/ignorados", async () => {
    const resolver = {
      resolve: async () => [
        { id: "ok", modulePath: "ok", enabled: true, source: "config" as const },
        { id: "fail", modulePath: "fail", enabled: true, source: "config" as const },
        { id: "skip", modulePath: "skip", enabled: false, source: "config" as const },
      ],
    };
    ...
    const manager = new PluginManager(resolver as never, loader as never, process.cwd());
