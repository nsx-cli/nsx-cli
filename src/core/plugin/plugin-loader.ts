...
export class PluginLoader {
  constructor(
    ...
    private readonly importer: Importer = async (modulePath: string) =>
      import(modulePath),
  ) {}
  ...
  public async load(
    ...
    const definition = await this.resolvePluginDefinition(
      moduleRef,
      descriptor.options ?? {},
    );
  ...
  private async resolvePluginDefinition(
    moduleRef: unknown,
  ...
  private validateDefinition(
    ...
    if (!definition || typeof definition !== 'object') {
      throw new Error(
        `Plugin inválido em ${modulePath}: export esperado é um objeto PluginDefinition.`,
      );
    }

    if (!definition.name || definition.name.trim().length === 0) {
      throw new Error(
        `Plugin inválido em ${modulePath}: campo 'name' é obrigatório.`,
      );
    }
  }
}
