...
export function resolveModuleMetadataObject(
...
export function ensureArrayProperty(
  ...
  if (
    ...
    !Node.isPropertyAssignment(resolvedProperty)
  ) {
...
export function ensureNamedImport(
  ...
  const declaration = sourceFile.getImportDeclaration(
    (entry) => entry.getModuleSpecifierValue() === moduleSpecifier,
  );
  ...
  const hasImport = declaration
    .getNamedImports()
    .some((entry) => entry.getName() === importName);
