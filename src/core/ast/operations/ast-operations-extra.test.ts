...
describe("AST operations extra", () => {
  it("adiciona import nomeado/default sem duplicar", () => {
    ...
    expect(declaration?.getNamedImports().map((entry) => entry.getName())).toEqual(["Dep"]);
...
export class UserModule {}`,
    ...
    expect(importsArray.getElements().map((entry) => entry.getText())).toEqual(["SharedModule"]);
  ...
  it("falha quando não encontra classe com @Module", () => {
    ...
    expect(() => resolveModuleMetadataObject(sourceFile)).toThrow("Module class with @Module decorator was not found");
  });
});
