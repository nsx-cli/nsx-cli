...
export class UsersModule {}`
    ...
    projectContext.getSourceFile = (filePath: string) => (filePath === sourceFile.getFilePath() ? sourceFile : undefined);
...
export class UsersModule {}`,
    ...
    expect(service.findSourceFile({ filePath: sourceFile.getFilePath() })).toBe(sourceFile);
    expect(service.findClass({ sourceFile, className: "UsersModule" })?.getName()).toBe("UsersModule");
    expect(service.findDecorator({ target: sourceFile.getClassOrThrow("UsersModule"), decoratorName: "Module" })).toBeDefined();
