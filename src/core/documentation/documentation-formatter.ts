  ...
  public format(snapshot: DocumentationSnapshot, outputPath: string): string {
    ...
    if (snapshot.routes.length === 0) {
      ...
      for (const route of snapshot.routes) {
        ...
        lines.push(`- ${route.controller} -> /${route.basePath} (${normalizedPath})`);
      }
    }
    ...
    lines.push(`- Models (${snapshot.prisma.models.length}): ${snapshot.prisma.models.join(", ") || "none"}`);
    lines.push(`- Enums (${snapshot.prisma.enums.length}): ${snapshot.prisma.enums.join(", ") || "none"}`);
