...
describe('TemplateMarketplaceService', () => {
  it('lista, pesquisa e instala templates remotos', async () => {
    ...
    const fetchMock = vi.fn(async (input: string) => {
            ...
            packs: [
              {
                ...
                templates: [
                  ...
                  {
                    name: 'service',
                    downloadUrl: 'https://example.com/service.hbs',
                  },
                ],
              },
            ],
      ...
      if (input === 'https://example.com/controller.hbs') {
        return new Response('export class {{controllerName}} {}', {
          status: 200,
        });
      }
    ...
    const installedController = await readFile(
      installResult.templates[0].filePath,
      'utf8',
    );
    ...
    expect(manifest.overrides.controller).toContain(
      path.join('.nsx', 'templates', 'enterprise-controller'),
    );
    expect(manifest.overrides.service).toContain(
      path.join('.nsx', 'templates', 'enterprise-controller'),
    );
  });
  ...
  it('lista instalados sem manifest e retorna vazio quando não há correspondência', async () => {
    const tempRoot = await mkdtemp(
      path.join(os.tmpdir(), 'nsx-marketplace-empty-'),
    );

    const fetchMock = vi.fn(
      async () => new Response(JSON.stringify({ packs: [] }), { status: 200 }),
    );
    const service = new TemplateMarketplaceService(
      tempRoot,
      new FileService(),
