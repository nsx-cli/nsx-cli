...
export class DocumentationCommand {
  ...
  public register(program: Command): void {
    const docs = program.command("docs").description("Geração de documentação do projeto");
    ...
    docs
      ...
      .action(async (options: { output?: string }) => {
        const result = await this.documentationService.generate({ outputPath: options.output });
