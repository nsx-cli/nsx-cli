...
export class MarketplaceCommand {
  ...
  public register(program: Command): void {
    const marketplace = program
      .command('marketplace')
      .description('Marketplace de templates remotos');
    const templates = marketplace
      .command('templates')
      .description('Explora e instala templates remotos');
    ...
    templates
      ...
      .action(async (options: { catalogUrl?: string }) => {
        ...
        for (const pack of result.packs) {
          ...
          console.log(
            `    templates: ${pack.templates.map((template) => template.name).join(', ')}`,
          );
        }
      });
    ...
    templates
      ...
      .action(async (packId: string, options: { catalogUrl?: string }) => {
        const result = await this.marketplaceService.install(packId, {
          catalogUrl: options.catalogUrl,
        });

        console.log(
          `Template pack instalado: ${result.pack.name} (${result.pack.id})`,
        );

        for (const template of result.templates) {
          console.log(
            `  - ${template.name} -> ${template.filePath.replace(/\\/g, '/')}`,
          );
        }

        console.log(
          `Manifest atualizado em ${result.manifestPath.replace(/\\/g, '/')}`,
        );
      });
